...
<script type="text/javascript">
        ...
        test('exec inline command', function() {
            $.each(['bold', 'italic', 'underline', 'strikethrough'], function() {
                assertCommand($.telerik.editor.FormatCommand, this);
            });
        });
        
        test('exec unordered list', function() {
            assertCommand($.telerik.editor.ListCommand, 'insertUnorderedList');
        });
        
        test('exec ordered list', function() {
            assertCommand($.telerik.editor.ListCommand, 'insertOrderedList');
        });
        ...
        test('exec insertLineBreak creates newLineCommand', function() {
            assertCommand($.telerik.editor.NewLineCommand, 'insertLineBreak');
        });
        
        test('exec insertParagraph creates paragraph command', function() {
            assertCommand($.telerik.editor.ParagraphCommand, 'insertParagraph');
        });

        test('exec createLink creates LinkCommand', function() {
            assertCommand($.telerik.editor.LinkCommand, 'createLink');
        });
        
        test('exec unlink creates UnlinkCommand', function() {
            assertCommand($.telerik.editor.UnlinkCommand, 'unlink');
        });
       
        test('exec insertImage creates image command', function() {
            assertCommand($.telerik.editor.ImageCommand, 'insertImage');
        });

        test('exec indent creates indent command', function() {
            assertCommand($.telerik.editor.IndentCommand, 'indent');
        });

        test('exec outdent creates indent command', function() {
            assertCommand($.telerik.editor.OutdentCommand, 'outdent');
        });
