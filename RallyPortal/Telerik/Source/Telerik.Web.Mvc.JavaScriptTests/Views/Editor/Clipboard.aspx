...
<script type="text/javascript">
    ...
    test('obsolete tags get rewritten', function() {
        var range = createRangeFromText(editor, '|foo|');
        editor.selectRange(range);
        clipboard.paste('<u>foo</u>');

        equal(editor.value(), '<span style="text-decoration:underline;">foo</span>');
    });

    test('spans get consolidated', function() {
        var range = createRangeFromText(editor, '|foo|');
        editor.selectRange(range);
        clipboard.paste('<span style="color: red;"><span style="text-decoration: underline;">foo</span></u>');

        var cleanDom = $(editor.value());

        equal(cleanDom.find("span").length, 0);
        equal(cleanDom.css("text-decoration"), "underline");
    });
