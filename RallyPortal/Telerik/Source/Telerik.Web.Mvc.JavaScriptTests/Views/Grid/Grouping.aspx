...
<script type="text/javascript">
        ...
        test('grouping creates grouping hidden', function() {
            ...
            grid.group('Name');
            grid.normalizeColumns(grid.groups.length + grid.columns.length - 1);
        ...
        test('rebind clears groups', function() {
            var grid = getGrid("#Grid1");
            grid.ajaxRequest = function() {};
                
            grid.rebind();
            equal(grid.groupBy, '');
            ok(!grid.groups.length);
        });

        test('rebind clears group header', function() {
            var grid = getGrid("#Grid1");
            grid.ajaxRequest = function() {};
                
            grid.rebind();
            equal(grid.$groupHeader.html(), grid.localization.groupHint);
        });
</script>
