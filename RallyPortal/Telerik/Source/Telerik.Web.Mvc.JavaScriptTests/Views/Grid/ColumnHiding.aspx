...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <% Html.Telerik().Grid<Customer>()
            ...
            .Columns(columns =>
            {
                columns.Bound(c => c.Name).Width(100).IncludeInContextMenu(false);
                columns.Bound(c => c.IntegerValue);                
            })            
            .DataBinding(binding => binding.Ajax())
            .ColumnContextMenu()
...
<script type="text/javascript">
        ...
        test("column is not included in the context menu", function() {
            var grid = getGrid("#Grid5"),
                menu;

            grid.$header.closest(".t-grid-header").trigger("contextmenu");
            menu = $("#Grid5_contextMenu");            
            equal(menu.find(":checkbox").length, 1);
        });
</script>
